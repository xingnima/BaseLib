...
public class DeviceUtil {
    ...
    public static boolean getMetaProxyState() {
        boolean proxy = false;
        try {
            PackageManager packageManager = BaseApplication.instance().getPackageManager();
            ApplicationInfo appInfo = packageManager.getApplicationInfo(BaseApplication.instance().getPackageName(), PackageManager.GET_META_DATA);
            proxy = appInfo.metaData.getBoolean("PROXY_STATE");
        } catch (PackageManager.NameNotFoundException e) {
            e.printStackTrace();
        }
        return proxy;
     ...
     * 获取友盟 appKey
     */
    public static String getMetaUmengAppKey() {
        String apiHost = "";
        try {
            PackageManager packageManager = BaseApplication.instance().getPackageManager();
            ApplicationInfo appInfo = packageManager.getApplicationInfo(BaseApplication.instance().getPackageName(), PackageManager.GET_META_DATA);
            apiHost = appInfo.metaData.getString("UM_APP_KEY");
        } catch (PackageManager.NameNotFoundException e) {
            e.printStackTrace();
        }
        return apiHost;
    ...
    public static String getUUID() {
        String uuid = SpUtil.getInstance().getString("uuid", "");
        if (TextUtils.isEmpty(uuid)) {
            uuid = UUID.randomUUID().toString();
            SpUtil.getInstance().putString("uuid", uuid);
        }
        return uuid;
    }
